...
struct CorruptedWorldMapView: View {
    ...
    private func convert(_ node: CorruptedNode) -> WorldNode {
        WorldNode(
            id: node.id,
            image: node.image,
    ...
    fileprivate func point(for node: CorruptedNode, in geo: GeometryProxy)
        -> CGPoint
    {
    ...
    fileprivate func nodesView(in geo: GeometryProxy) -> some View {
        ZStack {
            ForEach(world.worldNodes) { node in
                ...
                let scale: CGFloat = isFocused ? (pulse ? 1.25 : 1.15) : 1.0
